...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!student) {
      return NextResponse.json({ success: true, data: { student: null, scores: [], gpa: 0 } });
    }

    const data = await getStudentAcademicResults(user.schoolId, student.id);
    return NextResponse.json({ success: true, data });
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Failed to fetch student academic results";
